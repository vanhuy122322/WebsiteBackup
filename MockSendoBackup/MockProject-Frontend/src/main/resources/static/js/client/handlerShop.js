...
function updateCheckout(value) {
  ...
  if (confirm("Bạn muốn cập nhật đơn hàng này không?")) {
    $.post("/shop/checkout/update-status", { checkoutCode }, (resp) => {
...
function exportExcel() {
  ...
  $.get("/shop/export-excel", { name }, (resp) => {
  });
}
...
function loadImage(){
	let image=$('#image').val();
	  $("#image-product").html("");
	  $("#image-product").append(`
       <img alt="" src="${image}" style="width:200px;height:200px;">
      `);
}
...
function productDelete(id) {
  ...
  if (confirm("Bạn muốn xóa không?")) {
    $.post("/shop/product/delete", { id }, () => {
      document.location.href = "/shop/product/"+name;
    });
  }
}
